...
class Profil extends Component {
    ...
    get_user = () => {
        ...
        // console.log(items)
        let url = "http://localhost/tokonline/public/user/" + id;
        axios.get(url)
            .then(response => {
    ...
    get_alamat = () => {
        ...
        let id = JSON.parse(localStorage.getItem('id'))
        // console.log(items)
        let url = "http://localhost/tokonline/public/address/" + id;
        axios.get(url)
            .then(response => {
    ...
    Save_alamat = (event) => {
        ...
        let url = "http://localhost/tokonline/public/address/save";
    ...
    Drop_alamat = (id_alamat) => {
        if (window.confirm("Apakah anda yakin ingin menghapus data ini?")) {
            // $("loading").toast("show");
            let url = "http://localhost/tokonline/public/address/drop/" + id_alamat;
            axios
                .delete(url)
                .then(response => {
                    $("#loading").toast("hide");
    ...
    Save = (event) => {
        ...
        let url = "http://localhost/tokonline/public/user/save_profil";
        let form = new FormData();
        form.append("action", this.state.action);
                                    ...
                                    {user.map((item, index) => {
                                        return (
                                            <ul class="list-group" key={index}>
                                                <img className="rounded float-left" src={'http://localhost/tokonline/public/image/' + item.image} style={{ height: "240px", width: "200px" }} />
                                            </ul>
                                        )
                                    })}
                                </div>
                                ...
                                <Modal id="modal_user" title="Form User" bg_header="success"
                                    ...
                                    <form onSubmit={this.Save}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <select className="form-control" name="jenis_kelamin" value={this.state.jenis_kelamin} onChange={this.bind} required>
                                                <option value=" "></option>
